...
from wtforms.validators import Required, Length, Email, DataRequired, EqualTo, \
    Regexp
...
class JobDateForm(FlaskForm):
    ...
    run_date = StringField('计划运行时间', default="2019-03-20 11:11:11",
                           validators=[Required()])
...
class JobCronForm(FlaskForm):
    ...
    func_cmd = StringField('执行函数或命令', validators=[Required()])
    cron_date = StringField('计划运行时间(秒 分 时 日 月 周)',
                            default="20 10-15 11-15 * * *",
                            validators=[Required()])
...
class JobIntervalForm(FlaskForm):
    job_id = StringField('任务名称', default="INTERVAL-",
                         validators=[Required()])
    func_cmd = StringField('执行函数或命令', validators=[Required()])
    interval_time = StringField('计划运行时间(秒 分 时 日 周)',
                                default="10 0 0 0 0", validators=[Required()])
